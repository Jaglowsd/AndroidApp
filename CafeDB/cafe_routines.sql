...
/*!50001 VIEW `customerorders` AS select `item`.`iditem` AS `iditem`,`item`.`ordernumber` AS `ordernumber`,`product`.`name` AS `name` from ((`invoice` join `item` on((`item`.`ordernumber` = `invoice`.`idinvoice`))) join `product` on((`item`.`product` = `product`.`idproduct`))) */;
